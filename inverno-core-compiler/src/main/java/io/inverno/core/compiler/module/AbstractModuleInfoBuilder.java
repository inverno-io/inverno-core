...
abstract class AbstractModuleInfoBuilder implements ModuleInfoBuilder {
	...
	public AbstractModuleInfoBuilder(ProcessingEnvironment processingEnvironment, ModuleElement moduleElement) {
		...
		if(moduleType != null) {
			if(moduleType.getSuperclass().toString().equals("io.inverno.core.v1.Module")) {
				this.version = 1;
			}
			else {
				throw new IllegalStateException("Class " + this.moduleQName.getClassName() + " is not recognized as an Inverno module class which might indicate a name collision");
			}
		}
		else {
			this.version = InvernoCompiler.VERSION;
		}
	}
