 ...
 *
 */
public class TestLazyBeanSocket extends AbstractCoreInvernoTest {

	private static final String MODULEA = "io.inverno.core.test.lazy.moduleA";
	private static final String MODULEB = "io.inverno.core.test.lazy.moduleB";
	private static final String MODULEC = "io.inverno.core.test.lazy.moduleC";
	private static final String MODULED = "io.inverno.core.test.lazy.moduleD";
	
	@Test
	public void testSingleBeanInModule() throws IOException, InvernoCompilationException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException {
	...
	@Test
	public void testSingleBeanInComponentModule() throws IOException, InvernoCompilationException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException {
		InvernoModuleProxy moduleProxy = this.getInvernoCompiler().compile(MODULEA, MODULEB).load(MODULEB).build();
	...
	@Test
	public void testSingleBeanInModuleSocket() throws IOException, InvernoCompilationException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException {
		Runnable r = () -> System.out.println("test");
		InvernoModuleProxy moduleProxy = this.getInvernoCompiler().compile(MODULEC).load(MODULEC).dependencies(r).build();
	...
	@Test
	public void testMultiBeanInModuleSocket() throws IOException, InvernoCompilationException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException {
		Runnable r = () -> System.out.println("test");
		InvernoModuleProxy moduleProxy = this.getInvernoCompiler().compile(MODULEA, MODULED).load(MODULED).dependencies(r).build();
		...
		for(Runnable runnable : beanA_runnables1_list) {
			if(runnable.getClass().getCanonicalName().equals("io.inverno.core.test.lazy.moduleA.SomeRunnable")) {
				beanA_someRunnable1 = runnable;
			}
			else if(runnable.getClass().getCanonicalName().equals("io.inverno.core.test.lazy.moduleD.MyRunnable")) {
