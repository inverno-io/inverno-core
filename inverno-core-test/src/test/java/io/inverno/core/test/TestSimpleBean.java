 ...
 *
 */
public class TestSimpleBean extends AbstractCoreInvernoTest {

	private static final String MODULE = "io.inverno.core.test.simplebean";
	
	@Test
	public void testBeanCreation() throws IOException, InvernoCompilationException, IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException {
		Runnable someRunnable = () -> {};
		
		InvernoModuleProxy simpleProxy = this.getInvernoCompiler().compile(MODULE).load(MODULE).dependencies(someRunnable).build();
